...
AddEventHandler('KGCore:Client:OnPlayerLoaded', function()
    ...
    if player.metadata.tracker then
        ...
        TriggerEvent('kg-clothing:client:loadOutfit', trackerClothingData)
    else
        local trackerClothingData = {
            outfitData = {
                ['accessory'] = {
...
RegisterNetEvent('KGCore:Client:SetDuty', function(newDuty)
...
RegisterNetEvent('police:client:sendBillingMail', function(amount)
    SetTimeout(math.random(2500, 4000), function()
        ...
        if KGCore.Functions.GetPlayerData().charinfo.gender == 1 then
            gender = Lang:t('info.mrs')
        end
        local charinfo = KGCore.Functions.GetPlayerData().charinfo
...
RegisterNetEvent('police:client:policeAlert', function(coords, text)
    ...
    KGCore.Functions.Notify({ text = text, caption = street1name .. ' ' .. street2name }, 'police')
