...
local function IsTargetDead(playerId)
    ...
    KGCore.Functions.TriggerCallback('police:server:isPlayerDead', function(result)
        retval = result
        hasReturned = true
    end, playerId)
...
RegisterNetEvent('police:client:SeizeCash', function()
    local player, distance = KGCore.Functions.GetClosestPlayer()
...
RegisterNetEvent('police:client:RobPlayer', function()
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        if IsEntityPlayingAnim(playerPed, 'missminuteman_1ig_2', 'handsup_base', 3) or IsEntityPlayingAnim(playerPed, 'mp_arresting', 'idle', 3) or IsTargetDead(playerId) then
            KGCore.Functions.Progressbar('robbing_player', Lang:t('progressbar.robbing'), math.random(5000, 7000), false, true, {
...
RegisterNetEvent('police:client:JailPlayer', function()
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        if tonumber(dialog['jailtime']) > 0 then
            ...
            KGCore.Functions.Notify(Lang:t('error.time_higher'), 'error')
        end
    else
        KGCore.Functions.Notify(Lang:t('error.none_nearby'), 'error')
    end
...
RegisterNetEvent('police:client:BillPlayer', function()
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        if tonumber(dialog['bill']) > 0 then
            ...
            KGCore.Functions.Notify(Lang:t('error.amount_higher'), 'error')
        end
    else
        KGCore.Functions.Notify(Lang:t('error.none_nearby'), 'error')
    end
    ...
    if not IsPedRagdoll(PlayerPedId()) then
        ...
        if player ~= -1 and distance < 1.5 then
            ...
            if not IsPedInAnyVehicle(GetPlayerPed(player)) and not IsPedInAnyVehicle(PlayerPedId()) then
                ...
                KGCore.Functions.Notify(Lang:t('error.vehicle_cuff'), 'error')
            end
        else
            KGCore.Functions.Notify(Lang:t('error.none_nearby'), 'error')
        end
...
RegisterNetEvent('police:client:CuffPlayer', function()
    if not IsPedRagdoll(PlayerPedId()) then
        local player, distance = KGCore.Functions.GetClosestPlayer()
        if player ~= -1 and distance < 1.5 then
            local result = KGCore.Functions.HasItem(Config.HandCuffItem)
            if result then
                local playerId = GetPlayerServerId(player)
                if not IsPedInAnyVehicle(GetPlayerPed(player)) and not IsPedInAnyVehicle(PlayerPedId()) then
                ...
                KGCore.Functions.Notify(Lang:t('error.no_cuff'), 'error')
            end
        else
            KGCore.Functions.Notify(Lang:t('error.none_nearby'), 'error')
        end
...
RegisterNetEvent('police:client:GetKidnappedDragger', function()
    KGCore.Functions.GetPlayerData(function(_)
        ...
        TriggerEvent('kg-kidnapping:client:SetKidnapping', isEscorting)
    end)
end)
...
RegisterNetEvent('police:client:GetCuffed', function(playerId, isSoftcuff)
    ...
    if not isHandcuffed then
        ...
        if not isSoftcuff then
            ...
            KGCore.Functions.Notify(Lang:t('info.cuff'), 'primary')
        else
            cuffType = 49
            GetCuffedAnimation(playerId)
            KGCore.Functions.Notify(Lang:t('info.cuffed_walk'), 'primary')
        end
    else
        isHandcuffed = false
        isEscorted = false
        ...
        ClearPedTasksImmediately(ped)
        TriggerServerEvent('InteractSound_SV:PlayOnSource', 'Uncuff', 0.2)
        KGCore.Functions.Notify(Lang:t('success.uncuffed'), 'success')
    end
end)
...
CreateThread(function()
    while true do
        ...
        if isHandcuffed then
            ...
            if (not IsEntityPlayingAnim(PlayerPedId(), 'mp_arresting', 'idle', 3) and not IsEntityPlayingAnim(PlayerPedId(), 'mp_arrest_paired', 'crook_p2_back_right', 3)) and not KGCore.Functions.GetPlayerData().metadata['isdead'] then
