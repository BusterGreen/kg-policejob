...
function TakeOutImpound(vehicle)
    ...
    if coords then
        KGCore.Functions.TriggerCallback('KGCore:Server:SpawnVehicle', function(netId)
            ...
            KGCore.Functions.SetVehicleProperties(veh, json.decode(vehicle.mods))
        ...
        KGCore.Functions.TriggerCallback('KGCore:Server:SpawnVehicle', function(netId)
            ...
            if Config.VehicleSettings[vehicleInfo] ~= nil then
                if Config.VehicleSettings[vehicleInfo].extras ~= nil then
                    KGCore.Shared.SetDefaultVehicleExtras(veh, Config.VehicleSettings[vehicleInfo].extras)
                end
...
function MenuGarage(currentSelection)
    ...
    local playerGrade = KGCore.Functions.GetPlayerData().job.grade.level
...
function MenuImpound(currentSelection)
    ...
    KGCore.Functions.TriggerCallback('police:GetImpoundedVehicles', function(result)
        local shouldContinue = false
        if result == nil then
            KGCore.Functions.Notify(Lang:t('error.no_impound'), 'error', 5000)
        else
            shouldContinue = true
            for _, v in pairs(result) do
                local enginePercent = KGCore.Shared.Round(v.engine / 10, 0)
                local currentFuel = v.fuel
                local vname = KGCore.Shared.Vehicles[v.vehicle].name
        ...
        if shouldContinue then
            ...
            exports['kg-menu']:openMenu(impoundMenu)
        end
    end)
end
...
RegisterNetEvent('police:client:ImpoundVehicle', function(fullImpound, price)
    ...
    if vehicle ~= 0 and vehicle then
        ...
        if #(pos - vehpos) < 5.0 and not IsPedInAnyVehicle(ped) then
            KGCore.Functions.Progressbar('impound', Lang:t('progressbar.impound'), 5000, false, true, {
                ...
                TriggerEvent('KGCore:Notify', Lang:t('success.impounded'), 'success')
                ClearPedTasks(ped)
            end, function() -- Play When Cancel
                ClearPedTasks(ped)
                TriggerEvent('KGCore:Notify', Lang:t('error.canceled'), 'error')
            end)
        end
    end
end)
    ...
    KGCore.Functions.GetPlayerData(function(PlayerData)
        if PlayerData.job.type == 'leo' then
            ...
            if player ~= -1 and distance < 5.0 then
                ...
                KGCore.Functions.TriggerCallback('police:GetPlayerStatus', function(result)
                    if result then
                        for _, v in pairs(result) do
                            KGCore.Functions.Notify('' .. v .. '')
                        end
                    end
                end, playerId)
            else
                KGCore.Functions.Notify(Lang:t('error.none_nearby'), 'error')
            end
        end
    end)
end)
...
RegisterNetEvent('police:client:EvidenceStashDrawer', function()
    ...
    if #(pos - takeLoc) <= 1.0 then
        local drawer = exports['kg-input']:ShowInput({
        ...
        if drawer then
            ...
            TriggerServerEvent('kg-policejob:server:evidence', Lang:t('info.current_evidence', { value = currentEvidence, value2 = drawer.slot }))
        end
    else
        exports['kg-menu']:closeMenu()
    end
...
RegisterNetEvent('kg-policejob:ToggleDuty', function()
    TriggerServerEvent('KGCore:ToggleDuty')
    TriggerServerEvent('police:server:UpdateCurrentCops')
    TriggerServerEvent('police:server:UpdateBlips')
end)

RegisterNetEvent('kg-police:client:scanFingerPrint', function()
    local player, distance = GetClosestPlayer()
...
RegisterNetEvent('kg-police:client:spawnHelicopter', function(k)
    ...
    CreateThread(function()
        while true do
            ...
            if inStash and PlayerJob.type == 'leo' then
                ...
                if IsControlJustReleased(0, 38) then
                    TriggerServerEvent('kg-policejob:server:stash')
                    break
                end
            else
                break
    ...
    CreateThread(function()
        while true do
            ...
            if inTrash and PlayerJob.type == 'leo' then
                if PlayerJob.onduty then sleep = 5 end
                if IsControlJustReleased(0, 38) then
                    TriggerServerEvent('kg-policejob:server:trash')
                    break
                end
            else
                break
    ...
    CreateThread(function()
        while true do
            ...
            if inFingerprint and PlayerJob.type == 'leo' then
                if PlayerJob.onduty then sleep = 5 end
                if IsControlJustReleased(0, 38) then
                    TriggerEvent('kg-police:client:scanFingerPrint')
                    break
                end
            else
                break
    ...
    CreateThread(function()
        while true do
            ...
            if inHelicopter and PlayerJob.type == 'leo' then
                if PlayerJob.onduty then sleep = 5 end
                if IsControlJustReleased(0, 38) then
                    TriggerEvent('kg-police:client:spawnHelicopter')
                    break
                end
            else
                break
...
if Config.UseTarget then
    CreateThread(function()
        ...
        for i = 1, #Config.Locations['duty'] do
            ...
            exports['kg-target']:AddCircleZone('PoliceDuty_' .. i, vector3(v.x, v.y, v.z), 0.5, {
                name = 'PoliceDuty_' .. i,
                useZ = true,
                debugPoly = false,
            }, {
        ...
        for i = 1, #Config.Locations['stash'] do
            ...
            exports['kg-target']:AddCircleZone('PoliceStash_' .. i, vector3(v.x, v.y, v.z), 1.0, {
                name = 'PoliceStash_' .. i,
                useZ = true,
                debugPoly = false,
            }, {
                options = {
                    {
                        type = 'server',
                        event = 'kg-policejob:server:stash',
                        icon = 'fas fa-dungeon',
        ...
        for i = 1, #Config.Locations['trash'] do
            ...
            exports['kg-target']:AddCircleZone('PoliceTrash_' .. i, vector3(v.x, v.y, v.z), 0.5, {
                name = 'PoliceTrash_' .. i,
                useZ = true,
                debugPoly = false,
            }, {
                options = {
                    {
                        type = 'server',
                        event = 'kg-policejob:server:trash',
                        icon = 'fas fa-trash',
        ...
        for i = 1, #Config.Locations['fingerprint'] do
            ...
            exports['kg-target']:AddCircleZone('PoliceFingerprint_' .. i, vector3(v.x, v.y, v.z), 0.5, {
                ...
                options = {
                    {
                        type = 'client',
                        event = 'kg-police:client:scanFingerPrint',
        ...
        for i = 1, #Config.Locations['evidence'] do
            local v = Config.Locations['evidence'][i]
            exports['kg-target']:AddCircleZone('PoliceEvidence_' .. i, vector3(v.x, v.y, v.z), 0.5, {
    ...
    stashCombo:onPlayerInOut(function(isPointInside, _, _)
        if isPointInside then
            ...
            if PlayerJob.type == 'leo' and PlayerJob.onduty then
                exports['kg-core']:DrawText(Lang:t('info.stash_enter'), 'left')
                stash()
            end
        else
            inStash = false
    ...
    trashCombo:onPlayerInOut(function(isPointInside)
        if isPointInside then
            inTrash = true
            if PlayerJob.type == 'leo' and PlayerJob.onduty then
                exports['kg-core']:DrawText(Lang:t('info.trash_enter'), 'left')
                trash()
            end
        else
            inTrash = false
    ...
    fingerprintCombo:onPlayerInOut(function(isPointInside)
        if isPointInside then
            inFingerprint = true
            if PlayerJob.type == 'leo' and PlayerJob.onduty then
                exports['kg-core']:DrawText(Lang:t('info.scan_fingerprint'), 'left')
                fingerprint()
            end
        else
            inFingerprint = false
    ...
    evidenceCombo:onPlayerInOut(function(isPointInside)
        if isPointInside then
            inEvidence = true
            if PlayerJob.type == 'leo' and PlayerJob.onduty then
                exports['kg-core']:DrawText(Lang:t('info.evidence_stash_prompt'), 'left')
                evidence()
            end
        else
            inEvidence = false
...
CreateThread(function()
    ...
    helicopterCombo:onPlayerInOut(function(isPointInside)
        if isPointInside then
            ...
            if PlayerJob.type == 'leo' and PlayerJob.onduty then
                if IsPedInAnyVehicle(PlayerPedId(), false) then
                    exports['kg-core']:HideText()
                    exports['kg-core']:DrawText(Lang:t('info.store_heli'), 'left')
                    heli()
                else
                    exports['kg-core']:DrawText(Lang:t('info.take_heli'), 'left')
                    heli()
                end
            end
        else
    ...
    impoundCombo:onPlayerInOut(function(isPointInside, point)
        if isPointInside then
            ...
            if PlayerJob.type == 'leo' and PlayerJob.onduty then
                if IsPedInAnyVehicle(PlayerPedId(), false) then
                    exports['kg-core']:DrawText(Lang:t('info.impound_veh'), 'left')
                    impound()
                else
    ...
    garageCombo:onPlayerInOut(function(isPointInside, point)
        if isPointInside then
            ...
            if PlayerJob.type == 'leo' and PlayerJob.onduty then
                if IsPedInAnyVehicle(PlayerPedId(), false) then
                    exports['kg-core']:DrawText(Lang:t('info.store_veh'), 'left')
                    garage()
                else
                    local currentSelection = 0
